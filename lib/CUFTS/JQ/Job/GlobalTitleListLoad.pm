package CUFTS::JQ::Job::GlobalTitleListLoad;
...
sub work {
    ...
    eval {
        ...
        $self->work_schema->txn_do( sub {
            $self->debug('Loaded global resource: ' . $resource->name);
            $self->checkpoint( 0, 'Starting to load title list' );

            my $results = $resource->do_module( 'load_title_list', $self->work_schema, $resource, $file, 0, $self );
